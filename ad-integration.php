...
/*
Plugin Name: Active Directory Integration (SSO Version)
Version: 1.1.8
...
class ADIntegrationPlugin {
	...
	protected static $_all_options = array(
			...
			// SSO feature
			array('name' => 'AD_Integration_sso_enabled', 'type' => 'bool'),
	...
	public function initialize_options() {
		...
		if ( is_multisite() ) {
			if (is_super_admin()) {
				...
				// SSO feature
				add_site_option('AD_Integration_sso_enabled', false);
			...
			if (current_user_can('manage_options')) {
				...
				// SSO feature
				add_option('AD_Integration_sso_enabled', false);
	...
	public function register_adi_settings()
	{
		...
		register_setting('ADI-auth-settings', 'AD_Integration_sso_enabled', array(&$this, 'sanitize_bool'));
	...
	protected function _load_options() {
		...
		if ( is_multisite() ) {
			...
			// SSO feature
			$this->_sso_enabled 				= (bool)get_site_option('AD_Integration_sso_enabled');
 		...
 		if ( is_multisite() ) {
			...
			if ($arrPost['option_page'] == 'ADI-auth-settings') {			
				...
				update_site_option('AD_Integration_authorize_by_group', (bool)$arrPost['AD_Integration_authorize_by_group']);
				 ...
				 // SSO feature
				pdate_site_option('AD_Integration_sso_enabled', (bool)$arrPost['AD_Integration_sso_enabled']);
			}
